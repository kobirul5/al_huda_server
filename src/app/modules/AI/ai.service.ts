...
// Helper: Build system prompt for Islamic guidance
const buildSystemPrompt = (language: "en" | "bn" = "en", category?: string): string => {
  const languagePart =
    language === "bn"
...
const buildUserPrompt = (prompt: string, language: "en" | "bn" = "en"): string => {
  if (language === "bn") {
...
// Helper: Parse AI response and extract references
...
const getIslamicSuggestion = async (data: IIslamicSuggestionRequest): Promise<IAISuggestion> => {
  try {
    ...
    const systemPrompt = buildSystemPrompt(data.language || "en", data.category);
    const userPrompt = buildUserPrompt(data.prompt, data.language || "en");
...
const getQuranReference = async (query: string, language: "en" | "bn" = "en"): Promise<IReference[]> => {
  const suggestion = await getIslamicSuggestion({
...
const getHadithReference = async (query: string, language: "en" | "bn" = "en"): Promise<IReference[]> => {
  const suggestion = await getIslamicSuggestion({
